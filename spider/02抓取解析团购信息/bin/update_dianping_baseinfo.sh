...
function batch_parse_page_imp() {
	...
	ConfPath=/search/odin/Getpage/conf/
	...
	for host in ${NatHostList[@]}; do
		...
		$ScpFileExpect $LocalPath/offsum576 $host:$ConfPath "$Passwd"
...
function merge_online_urls_imp() {
	...
	rm -f $scanUrlFile;  cp $scanUrlFile.merge $scanUrlFile
}
...
function full_update_from_scan_shopurls() {
	...
	echo "clean local path done."

	# 对扫库清理出来的URL进行分离
	# 参考 bin/filte_city_type_urls.py 
	scanShopTypeUrls=Scan/data/shop_urls.types
	python bin/split_scan_shop_typeurl.py $scanShopTypeUrls $type
	echo "split dianping urls done."

	# 将线上存在的URL加入进来 追加到后面
	merge_online_urls $type
	echo "merge urls done."

	# 2. 扫库，解析页面
	echo "begin to batch parse page from db ..."
	batch_parse_page_from_db $type
	
	echo "batch parse page from db done."
}
...
# 全量扫库获取大众点评的数据  主要用这个就可以
full_update_from_scan_shopurls $type

# 将分布式扫库得到的结果拷贝到本地来
#scp_host_result_to_local $type
