...
function parseBaseInfo(url, content) {
	content = content.replace('<script type="text/panel" class="J-panels">', '');

	var $ = cheerio.load(content);
	
	var title = trim($('#body > div.body-content.clearfix > div.breadcrumb > span').text());
	...
	/*
	var commentCount="", avgPrice="", scoreTaste="", scoreCondition="", scoreService="";

	$('div.brief-info > span').each(function(){
		var text = trim($(this).text());
		console.log("text: " + text + "\t" + text.indexOf("条评论"));
		if (text.indexOf("条评论") !== -1) { commentCount = text.replace("条评论", "");}
		else if (text.indexOf("人均") !== -1) { avgPrice = text.replace(/(人均|消费|费用|元|：)/g, "");}
		else if (text.indexOf("口味") !== -1) { scoreTaste = text.replace(/(口味|：)/g, "");}
		else if (text.indexOf("环境") !== -1) { scoreCondition = text.replace(/(环境|：)/g, "");}
		else if (text.indexOf("服务") !== -1) { scoreService = text.replace(/(服务|：)/g, "");}
	});
	*/
	var downreduce = trim($('span.download-reduce').text());
	...
	var recommNoPhotoFood = '';
    	...
    	$('.recommend-name > a').each(function(){
	...
	pushResult('address', normalStr(address));
        ...
        pushResult('downreduce', downreduce);
	...
	// user comments
    	$("li.comment-item").each(function(){
		var userName = trim($(this).find('p.user-info > a.name').text());
            	var userPhoto = $(this).find('a.J-avatar > img').attr('data-lazyload');
		...
		// 图集
            	var photoArray = [];
            	var photos = $(this).find('a.J-photo > img').each(function() {
			//var photo = trim($(this).attr('src'));
			var photo = trim($(this).attr('data-lazyload'));
