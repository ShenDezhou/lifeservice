...
function parseBaseInfos(){
    ...
    */
    ...
    var city = $.trim($('h1.shopall > strong').text());
    if (city) {
        city = city.replace('美食地图', '');
    }
    pushResult('url', city + '@@@' + url);
    
    // 商区
    $('div.box.shopallCate h2:contains("商区")').nextAll().each(function(){
    ...
    // 菜系
    $('div.box.shopallCate dt>a:contains("美食")').parent().next().find('ul > li').each(function(){
        cookName = $(this).text();
        cookUrl = $(this).find('a').attr('href');
        key = 'cook\t' + city + '\t' + cookUrl;
        value = normalStr(cookName.replace("|", ""));
        pushResult(key, value);
    });
    ...
    // 地标
    $('div.box.shopallCate h2:contains("地标")').nextAll().each(function(){
       highLevelAreaName = $(this).find('dt').text();
       highLevelAreaUrl = $(this).find('dt').find('a').attr('href'); 
       $(this).find('dd>ul>li').each(function(){
            lowLevelAreaName = $(this).find('a').text();
