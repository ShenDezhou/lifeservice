...
function parseBaseInfos(){
	...
	/*
	var description = $.trim($('#overlayInnerDiv > div > div > p').text());
	if (description) {
		description = description.replace("描述：", "");
	}
	*/
	var type =  $.trim($('#HEADING_GROUP > div > div.heading_details').text());
    var descriptionList = [];
    descriptionList.push( $('div.details_wrapper div.listing_details').text() );
    descriptionList.push( $.trim($('.detail_section.details > div > p').text()));
	...
	pushResult('description', normalStr(takeFirst(descriptionList)));
