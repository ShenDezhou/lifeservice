...
function parseBaseInfos(){
    ...
    */
	...
	$('ul#performList > li').each(function(){
	    var title = $.trim($(this).find('h2').text());
	    var itemUrl = $(this).find('h2 > a').attr('href');
	    var img = $(this).find('p.img img').attr('src');
	    var price = $.trim($(this).find('p.price-sort').text());
	    var org = $.trim($(this).find('span.ml20:contains("场馆：")').find('a').text());
	    var date = $.trim($(this).find('p.mt5:contains("时间：")').text());
	    var start, end;
	    if (date) {
	        date = $.trim(date.replace('时间：', ""));
	        date = $.trim(date.replace(/场馆：.*/, ""));
	        dateArray = date.split('-');
	        if (dateArray.length === 2) {
	            start = $.trim(dateArray[0]);
	            end = $.trim(dateArray[1]);
	        } else {
	            start = date;  end = date;
	        }
	    }
	    
	    var itemInfo = title + "\t" + itemUrl + "\t" + img + "\t" + org + "\t" + date + "\t" + start + "\t" + end;
	    pushResult(url, itemInfo );
	});
