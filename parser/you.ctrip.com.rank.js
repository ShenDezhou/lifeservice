...
function parseBaseInfos(){
    ...
    */
    ...
    $('.list_mod2').each(function(){
        var url = $.trim($(this).find('div.leftimg > a').attr('href'));
    	var address = $.trim($(this).find('dd.ellipsis').text());
    	var rank = $.trim($(this).find('s').text()).replace(/[第名返]/g, '');
    	var avgConsume = $.trim( $(this).find('span.price').text() );
    	var score = $.trim( $(this).find('a.score > strong').text().replace(/返现.*$/, '') );
    	if (url && url.length > 5) {
    	    url = 'http://you.ctrip.com' + url;
    	    var value = url + '\t' + rank + '\t' + score + '\t' + address + '\t' + avgConsume;
