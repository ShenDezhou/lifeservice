...
class MovieParser:
	...
	def __init__(self, conf):
		...
		self.PRESALE_KEY = 'movie_presale'
		self.MOVIE_DETAIL_KEY = 'movie_detail'
		...
		self.movieItemList = []
		self.movieDetailItemList = []
		self.cinemaItemList = []
		self.CMRItemList = []
		self.presaleItemList = []
		...
		logging.basicConfig(level=logging.DEBUG,
                ...
                datefmt='%Y-%m-%d %H:%M:%S',
                filename='logs/MovieParser_' + today + '.log',
                filemode='a')
		logging.info('init MaoyanParser')
	...
	def printParseResult(self, file):
		...
		presaleOutput = 'data/' + filename + '_presale'
		movieDetailOutput = 'data/' + filename + '_movie_detail'
		movieFD = open(movieOutput, 'w')
		cinemaFD = open(cinemaOutput, 'w')
		relationFD = open(relationOutput, 'w')
		presaleFD = open(presaleOutput, 'w')
		movieDetailFD = open(movieDetailOutput, 'w')

		self.printItemList(self.movieItemList, movieFD)
		self.printItemList(self.cinemaItemList, cinemaFD)
		self.printItemList(self.presaleItemList, presaleFD)
		self.printItemList(self.movieDetailItemList, movieDetailFD)
	...
	def parseImp(self, content):
		...
		presaleInfoMap = self.commonExtractInfo(self.PRESALE_KEY, content)
		movieDetailInfoMap = self.commonExtractInfo(self.MOVIE_DETAIL_KEY, content)
		relationInfoMap = self.extractCinamaMovieRelInfo(content)
		
		# insert every movie & cinema & movie_presale item
		self.insertItem(movieInfoMap, movieKey, self.movieItemList)
		self.insertItem(cinemaInfoMap, cinemaKey, self.cinemaItemList)
		self.insertItem(presaleInfoMap, movie_presaleKey, self.presaleItemList)
		self.insertItem(movieDetailInfoMap, movie_detailKey, self.movieDetailItemList)
		# 排片场次信息，与上述两个不同
