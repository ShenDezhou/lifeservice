...
def download_all_city_deal_detail(cityList):
	...
	for cityChName, cityCode, cityEnName in cityList:
		...
		if os.path.exists(idListFile):
			idListFileList.append((cityEnName, idListFile))

	if len(idListFileList) == 0:
		logging.info('idlist file is empty.')
		return

	pool = ThreadPool(20)
	pool.map(download_all_city_deal_detail_imp, idListFileList)
	pool.close()
	pool.join()
	logging.info('get all city\'s deal items done.')




# 下载所有城市的团购信息全集
def download_all_city_deal_detail_imp(cityFilePair):
	if len(cityFilePair) != 2:
		return
	cityEnName, idlistFile = cityFilePair
	if not os.path.exists(idlistFile):
		return

	logging.info('begin to get %s\'s all deal...' % cityEnName)

	# 下载各城市的
	dealFile = getAllTuanFilePath(cityEnName)
	fd = open(dealFile, 'w+')
...
def getAllTuanFilePath(cityEnName):
	return 'tuandata/all/%s.data' % cityEnName
...
#cityFilePair = ('baishan', 'tuandata/all/baishan.idlist')
#download_all_city_deal_detail_imp(cityFilePair)
