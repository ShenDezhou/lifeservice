...
class Item:
	# 构造函数
	def __init__(self, id, title, domain, city, area, addr, poi):
		self.id = id
		self.poi = poi
		#self.tel = tel
		self.city = city
...
class CinemaMerger:
	...
	sep, telSep = '\t', '|'
	idRow, urlRow, titleRow, cityRow = -1, -1, -1, -1
	...
	def addrSimilar(self, curAddr, lastAddr):
		# 互为前后缀，则直接返回True
		if self.isAddrSame(curAddr, lastAddr):
			return True
	...
	# 两个地址是否一致
	def isAddrSame(self, curAddr, lastAddr):
		if curAddr == lastAddr:
			return True
		if curAddr.find(lastAddr) != -1 or lastAddr.find(curAddr) != -1:
			return True
		return False
	...
	def mergeByTel(self, file):
		...
		for line in open(file):
			...
			curCity, curArea = segs[self.cityRow], segs[self.areaRow]
	...
	def mergeByAddr(self, file):
		...
		for line in open(file):
			...
			if self.isAddrSame(curAddr, self.lastAddr):
				for lastItem in lastSameAddrList:
					if curCity == lastItem.city and curDomain != lastItem.domain:
						print 'addr\t' + curID + '\t' + lastItem.id
			else:
				lastSameAddrList = []

			self.lastAddr = curAddr
			curItem = Item(curID, curTitle, curDomain, curCity, curArea, curAddr, curPoi)
			lastSameAddrList.append(curItem)
		...
		for line in open(file):
			...
			if curTitle == self.lastTitle:
				for lastItem in lastSameTitleList:
					if curCity==lastItem.city and curArea==lastItem.area and curDomain!=lastItem.domain:
						# 如下任意一个相似即可
						isSimiliar = True
