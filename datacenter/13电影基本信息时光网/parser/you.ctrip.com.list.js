...
function parseBaseInfos(){
    ...
    */
    ...
    var country = $.trim($('div.dest_toptitle h1').text());
    var countryID = baseUrl.replace("http://you.ctrip.com/countrysightlist/", "");
    countryID = countryID.replace(/\/.*$/, "");
    
    $('.list_mod1').each(function(){
        var url = $.trim($(this).find('div.cityimg > a').attr('href'));
        var city = $.trim($(this).find('div.cityimg span').text() );
        var recommandSights = '';
        $(this).find('.ellipsis > a').each(function(){
    	...
    	if (url && url.length > 5) {
    	    url = url.replace('/place/', '');
    	    url = url.replace('.html', '');
            //var value = city + '\t' + recommandSights;
            //pushResult(url, value);
    	
    	    var key = country + '\t' + countryID + '\t' + city;
    	    var value = url;
