...
# 时光网各个任务的存放路径
MtimePath = '/search/liubing/spiderTask/result/liubing/'
ActorPath = MtimePath + 'task-121'
ImagePath = MtimePath + 'task-122'
VideoPath = MtimePath + 'task-123'
DescribePath = MtimePath + 'task-144'
SummaryFile = '/search/zhangk/Fuwu/Source/Crawler/beijing/movie/mtime_summary.add'
OnlineStatusFile = '/search/zhangk/Fuwu/Source/Crawler/beijing/movie/mtime_online.add'
...
def get_movie_list(output):
	...
	for key in onlineMovieDict:
		...
		fd.write(('http://movie.mtime.com/%s/\tonline\n' % key).encode(Coding, 'ignore'))
	fd.close()
...
def getMovieDict(file):
	...
	for line in open(file):
		...
		if len(segs) != 2:
			continue
		title, id = segs
		id = id.replace('http://movie.mtime.com/', '').replace('/', '')
		movieDict[id] = (title, '')
	return movieDict
...
def main():
	...
	movieDict = getMovieDict('/tmp/zzzz')
	# 获取电影的剧照
