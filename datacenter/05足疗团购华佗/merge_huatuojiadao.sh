...
function dispatch() {
	...
	for serviceFile in $(ls data/huatuojiadao_service.*); do
		...
		if [ $lines -gt 0 ]; then
			...
			rm -f $tuanDestFile.bak2;  mv $tuanDestFile $tuanDestFile.bak2
			awk -F'\t' '{
				if ($3=="华佗驾到") { next }
				print
			}' $tuanDestFile.bak2 > $tuanDestFile
			cat $serviceFile >> $tuanDestFile
		fi
	...
	LOG "dispatch huatuojiadao services done."
}
...
function get_huatuojiadao_services() {
	...
	LOG "get huatuojiadao services done."
}
