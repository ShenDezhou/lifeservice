...
def test(url, header):
	#url = 'http://www.51xiancheng.com/subject/1485'
	response = GetRequest(url, header)#.encode('utf8', 'ignore')
	response = response.replace('QQmap&&QQmap(', '').replace(')', '')
	resultDict = json.loads(response)
	print resultDict["result"]["address_component"]["province"]
	print resultDict["result"]["address_component"]["city"]
	print resultDict["result"]["address_component"]["district"]
