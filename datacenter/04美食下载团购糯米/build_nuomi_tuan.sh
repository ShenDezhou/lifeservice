...
function get_expired_tuanitems() {
	...
	for tuanFile in $(ls data/*_tuan); do
		...
		LOG "get $city's expired tuan items done."  >> $TUAN_LOG
	done
}
...
function fetch_expired_tuanitems_pages() {
	...
	LOG "fetch & parse all nuomi tuan items done."  >> $TUAN_LOG
}
...
function recall_noexpired_tuanitems() {
	for tuanFile in $(ls data/*_tuan); do
		...
		LOG "recall tuan items which not expired for $city done."  >> $TUAN_LOG
	done
}
