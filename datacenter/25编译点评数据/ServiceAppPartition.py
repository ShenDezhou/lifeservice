...
import keyConfig
import Normalizer
from Normalizer import DPCrumbNormlizer
...
logging.basicConfig(level=logging.DEBUG,
                ...
                datefmt='%Y-%m-%d %H:%M:%S',
                filename='logs/ServiceAppPartition_' + today + '.log',
                filemode='a')
...
class Normalize:
	...
	def __init__(self, normConf):
		...
		# 大众点评特有的处理面包线的类
		cityBusinessConf = 'conf/dianping_city_business_cook_conf'
		self.DPCrumbNormer = DPCrumbNormlizer(cityBusinessConf)
	...
	def loadNormConf(self):
		for line in open(self.normConf):
			...
			if len(segs) != 3:
				continue
			fileSuffix, key, normFun = segs[0], segs[1], segs[2]
			...
			if not hasattr(Normalizer, normFun):
				...
				continue
			self.normFunDict[fileSuffix + '\t' + key] = getattr(Normalizer, normFun)
			#print fileSuffix + '\t' + key, getattr(Normalizer, normFun)

	# 归一化
	def normalize(self, input):
		#if not (input.endswith('.baseinfo') or input.endswith('.comment') or input.endswith('.recomfood')):
		#	return

		fileSuffix = re.sub('.*\.', '', input)
		fileType = re.sub('^[^_]+_', '', input)
		...
		for line in open(input):
			...
			# 对大众点评的面包线数据单独处理
			if input.find('dianping') != -1 and key == 'breadcrumb':
				if fileType == 'restaurant':
					normKVDict = self.DPCrumbNormer.normlizeRestaurantCrumb(key, val)
				elif fileType == 'play':
					normKVDict = self.DPCrumbNormer.normlizePlayCrumb(key, val)
