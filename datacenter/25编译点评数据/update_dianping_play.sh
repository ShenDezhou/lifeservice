...
OfflinePlayPath=/fuwu/Source/offlinedb/play
OnlinePlayPath=/fuwu/Source/Input
PlayBackupPath=/fuwu/Source/history/play

# 合并 & 去重play数据
function merge_uniq_play() {
	for offlineFile in $(ls $OfflinePlayPath); do
		city=${offlineFile/_urls*/}
		onlineFile=${OnlinePlayPath}/${city}_play_dianping_detail
		...
		LOG "merge & uniq $city city file done."
	done
}
...
function scp_offline_play() {
	...
	OfflineDBPath=/search/fangzi/ServiceApp/Dianping/data/play/result
	LocalOfflineDBPath=/fuwu/Source/offlinedb/play/
	scp $OfflineDBRemoteHost:$OfflineDBPath/*result $LocalOfflineDBPath
	LOG "scp offline play file from $OfflineDBRemoteHost done."
}
...
function clean_temp_files() {
	...
	rm -f Input/*_${type}_dianping_detail.*norm.*
	rm -f Input/*_${type}_dianping_detail.*table.*
}
...
function build_dianping_play() {
	...
	# 去重
	for file in $(ls Input/*${type}_dianping_detail); do
		uniqShop $file
	...
	# 一个url对应多实例的情况
	for file in $(ls Input/*${type}_dianping_detail.recomfood.norm); do
		preHandle $file
	done
	for file in $(ls Input/*${type}_dianping_detail.comment.norm); do
		preHandle $file
	done

	# 转成表格式
	for file in $(ls Input/*${type}_dianping_detail.*.norm); do
...
function updatePlayShopID() {
	URL_ID_CONF="conf/play_url_id_conf"
	URL_ID_CONF_UPDATE="conf/play_url_id_conf.update"
...
function updateAllPlayShopID() {
	...
	for cityDir in $(ls Crawler/); do
		file="Input/${cityDir}_${type}_dianping_detail"
		# 检查一下该城市下是否有相关文件
...
function dispatch() {
	type="play"
	for srcFile in $(ls Input/*${type}*.table.id); do
