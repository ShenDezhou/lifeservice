...
function build_mtime_movie() {
	rm -f Input/*movie_*
	rm -f Input/*cinema*
	mtimeMoviePath="/search/zhangk/Fuwu/Source/Crawler/beijing/movie"
	
	# 预处理片花数据, 将片花对应电影的URL 从无线转成转PC的格式，用于后续合并使用
	awk -F'\t' '{
		key=$1; value=$2;
		if (key == "url") { 
		...
		print key "\t" value
	}' $mtimeMoviePath/mtime_videos > Input/movie_movie_videos
	
	# 构建猫眼电影与时光网电影的映射关系
	maoyanDescFile="$mtimeMoviePath/maoyan_shortdesc"
	detailFile="$mtimeMoviePath/mtime_detail"
	mapFile="tmp/maoyan_mtime_map"
	merge_maoyan_mtime $maoyanDescFile $detailFile $mapFile
	
	# 合并猫眼电影与时光网电影的短评信息
	mtimeDescFile="$mtimeMoviePath/mtime_shortdesc"
	mergeDescFile="tmp/shortdesc"
	merge_shortdesc $maoyanDescFile $mtimeDescFile $mapFile $mergeDescFile

	# 替换猫眼的评论数据里的URL
	commentFile="$mtimeMoviePath/maoyan_comments"
	output="Input/movie_movie_comments"
	replace_url_of_comments $mapFile $commentFile $output

	# 合并图集到detail文件中
	detailFile="$mtimeMoviePath/mtime_detail"
	photosFile="$mtimeMoviePath/mtime_photos"
	...
	# 替换完整summary; 归一化票房字段
	summaryFile="$mtimeMoviePath/mtime_summary"
	replace_summary_of_movie $summaryFile $detailFile
	

	# 为演员表文件添加title数据
	actorFile="$mtimeMoviePath/mtime_actors"
