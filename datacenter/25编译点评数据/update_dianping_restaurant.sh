...
function clean_tmp_files() {
	rm -f Input/*restaurant_dianping_detail.baseinfo*
	rm -f Input/*restaurant_dianping_detail.comment*
	rm -f Input/*restaurant_dianping_detail.photoset*
	rm -f Input/*restaurant_dianping_detail.recomfood*
	rm -f Input/*restaurant_dianping_detail.tuan*
	rm -f Input/*restaurant_dianping.*norm.*
	rm -f Input/*restaurant_dianping.*table.*
}
...
function updateRestaurantID() {
	...
	ARGIND == 3 {
		...
		# 外卖的链接
		if ($WaiROW != "") {
			gsub("http://waimai.dianping.com/dish/", "http://m.dianping.com/waimai/mindex#!detail/i=", $WaiROW)
		}
